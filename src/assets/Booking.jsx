...
export default function FlightCard() {
  return (
    <div className="flex flex-col items-center w-full bg-gradient-to-b from-white-50 to-white min-h-screen py-10">
      ...
      {/* Main layout */}
      <div className="flex flex-col md:flex-row md:flex-wrap justify-center items-start w-full max-w-7xl px-4 gap-10">
        {/* Sidebar buttons */}
        <div className="flex md:flex-row  flex-wrap gap-4">
          {['Flights', 'Hotels', 'Buses'].map((label, i) => (
            <button
              key={i}
              className="px-5 py-2 rounded-full border-2 border-blue-500 text-blue-600 font-semibold hover:bg-blue-500 hover:text-white transition duration-300"
            >
              {label}
            </button>
          ))}
